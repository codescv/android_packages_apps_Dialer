...
public class PinyinSmartDialMap implements SmartDialMap {
    ...
    private char getPinyinInitial(char ch) {
        String pyStr = HanziToPinyin.getInstance().getFullPinYin(String.valueOf(ch));
        char c = pyStr.toLowerCase().charAt(0);
        return c;
    }

    @Override
    public boolean isValidDialpadAlphabeticChar(char c) {
        char ch = getPinyinInitial(c);
        return (ch >= 'a' && ch <= 'z');
    }
